...
form flask import Flask, request, jsonify, render_template_string, abort
...
def find_first_video_id(query):
   try:
      ...
      "https://www.youtube.com/results?search_query="+urllib.parse.quote_plus(query),
       headers={"User-Agent":Mozillla/5.0","Accept-Language":en-US","Cookie":"SOCS=CAI"})
...
def buld__youtube_target(cmd):
    play="play" in cmd
    q=re.sub(r"(open youtube( and (play|research))?|play|search( for)?|on youtube)","",cmd).strip()
...
return jsonify(error="Only Youube and Gmail commands supported.")
